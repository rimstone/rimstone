...
// 
// RimStone SERVICE process manager. 
//
...
#define RIM_SHMEM "/rimmem"


// display error
#define RIM_FERR (errno == 0 ? "" : strerror(errno))

// assert not null (used for malloc)
#define RIM_ANN(x) ((x) == NULL ? exit_error(-1, "Cannot allocate memory [%s]", RIM_FERR) : 0)


#define RIM_MAX_ARGS 64 // max args to mrim in -a

#define RIM_MAX_FILELEN 200 // max path+file length

// client commands to server and back
#define RIM_OKAY 0
#define RIM_START 1
#define RIM_STOP 2
#define RIM_RESTART 3
#define RIM_QUIT 4
#define RIM_OKAY_STARTED 5
#define RIM_OKAY_RUNNING 6
#define RIM_STATUS 7
#define RIM_STOPONE 8
// codes for processes and manager communication
#define RIM_COMMAND 9
#define RIM_DONE 10
#define RIM_DONEBAD 11
#define RIM_DONESTARTUP 12
#define RIM_DONESTARTUP0 13

// invalid exit status of child process (used to tell us we don't know what happened)
#define RIM_BAD_EXIT 1000

// locks for checking server the only one running for an app
#define RIM_LOCK 1 // lock it
#define RIM_CHECKLOCK 2 // check if locked
#define RIM_MAX_CLI_WAIT 10 // max second client will wait to hear back from server

//
// now that we know the server is (most likely) running, do the handshake so we get the confirmation the job is done
// number of tries correlate to the sleep interval. Total time to wait is (RIM_MAX_CLI_WAIT * 1000), and since we're checking
// every mslp (400 millisecs) , total of (RIM_MAX_CLI_WAIT*1000)/mslp checks. We do +1 to get at least one try in case the total is 0.
#define RIM_TRIES ((rim_num)((RIM_MAX_CLI_WAIT*1000)/mslp)+1)
...
#define RIM_MOUT 0
#define RIM_MLOG 1
#define out_msg(...) log_msg0(RIM_MOUT, __VA_ARGS__) 
#define log_msg(...) log_msg0(RIM_MLOG, __VA_ARGS__)
...
typedef struct s_shbuf {
    volatile _Atomic rim_num command;
    volatile _Atomic rim_num data1;
} shbuf;
...
static char *usage_message =
    ...
    -h            this help\n\
    \n\
Type 'man mrim' for more help\n";

static rim_num maxblog = 400; // listening back log
static rim_num maxproc = 0;  // max-worker
static pid_t *plist; // list of process IDs started by mrim instance
static rim_num sockfd; // socket passed down to forked child
static rim_num num_process; // abs max # of processes, plist is sized on it
static FILE *logfile = NULL; // this is mrim.log
static pid_t sid = 0; // session id of the child group
static char *rimapp = ""; // app name
static rim_num num_to_start_min = 5; // min-worker
static rim_num adapt = 0; // adaptive mode
static rim_num tspike = 30; // number of secs to die down for client srvc process if no demand 
static rim_num port = 0; // port (tcp)
...
static shbuf *rimmem; // shared memory for communication btw client and server
static char *client_msg = "";  // message from client -m
static rim_num temp_no_restart = 0;  // by default, process stopped is restarted. 
                                 // if temp_no_restart is 1, then it won't be, if 0, it will be.
static rim_num modreload = 1; // reload if exec modified
static bool silent = false;
static char *parg = ""; // input args -a
static char *xarg[RIM_MAX_ARGS]; // list of parsed args -a
static rim_num mslp=400; /*millisecs to sleep*/ 
static rim_num lfd = -1; // locked file descriptor
static rim_num unix_sock = 0; // tcp vs unix sock
static rim_num initit=0; // initialize dirs
static rim_num islogging = 0; // logging or not
static rim_num showinfo = 0;  // show info -e
static rim_num dnr = 0;  // do not restart if 1
static rim_num exenotfound = 0; // print exe not found just once
static rim_num fg = 0; // if 1, run in foreground

// functions prototypes
static pid_t whatisprocess(pid_t p, int *st);
static void log_msg0(rim_num dest, char *format, ...);
static void exit_error(int ec, char *format, ...);
static void wait_server_end() ;
static void getshm();
static void checkshm(bool is_running_server);
static void cli_getshm(char *comm, rim_num byserver);
static void usage(int ec);
static void start_child (char *command, rim_num pcount);
static void processup(char addone);
static void tokarg();
static void handlestop(rim_num sig);
static void sleepabit(rim_num milli);
static void unlockfile (rim_num lf);
static rim_num testlockfile (char *fname) ;
static rim_num lockfile (char *fname, rim_num *lf);
static rim_num srvhere(rim_num op);
static void runasuser();
static void owned (char *ipath, uid_t uid);
static void initdir (char *ipath, rim_num mode, uid_t uid, gid_t guid);
static rim_num checkmod();
static void msg_notfound(char *err);
static rim_num timeq(struct timespec *a, struct timespec *b);
static rim_num connwait();
static rim_num totprocess();

//
// check if there are input connections from outside to create new srvc process
// existing srvc processes wait for requests too. If mrim gets to wait on an input connection
...
static rim_num checkmod() {
    ...
    struct stat sbuff;
    if (stat(command, &sbuff) != 0) {  msg_notfound(RIM_FERR); return 0;} // command could be temporarily deleted
...
//
static void initfile (char *ipath, rim_num mode, uid_t uid, gid_t guid) {
    ...
    // nothing's changing
    if (chown (ipath, uid, guid)!= 0) exit_error (-1, "Cannot change the ownership of file [%s], [%s]", ipath, RIM_FERR);
    if (chmod(ipath, mode) != 0) exit_error (-1, "Cannot set permissions for file [%s]", RIM_FERR);
}
...
static void initdir (char *ipath, rim_num mode, uid_t uid, gid_t guid) {
    ...
    if (mkdir (ipath, mode) != 0) if (errno != EEXIST) exit_error (-1, "Cannot create directory [%s], [%s]", ipath, RIM_FERR); 
    // We check if this isn't local install; if so, we will try chown;
    // We'll check if current user/group matches what we want to change to, and if not, emit error; otherwise continue since
    // nothing's changing
    if (chown (ipath, uid, guid)!= 0) exit_error (-1, "Cannot change the ownership of directory [%s], [%s]", ipath, RIM_FERR);
    if (chmod(ipath, mode) != 0) exit_error (-1, "Cannot set permissions for directory [%s]", RIM_FERR);
}
...
static void owned (char *ipath, uid_t uid) {
    ...
    if (stat(ipath, &sbuff) == 0) {
        if (sbuff.st_uid != uid) {
            if ((pwd = getpwuid (sbuff.st_uid)) == NULL) exit_error (-1,"Cannot find user who owns [%s], user id [%ld], [%s]", ipath, sbuff.st_uid, RIM_FERR);
            // must allocate new buffer as getpwuid results overwrite the old ones
            RIM_ANN (userown = strdup(pwd->pw_name));
            if ((pwd1 = getpwuid (uid)) == NULL) exit_error (-1,"Cannot find user [%ld], [%s]", ipath, uid, RIM_FERR);
            RIM_ANN (usernew= strdup(pwd->pw_name));
...
static void runasuser() {
    ...
    char ipath[RIM_MAX_OS_UDIR_LEN];
    rim_dir (RIM_DIR_APP, ipath, sizeof(ipath), rimapp, NULL);
    if (chdir(ipath) != 0) exit_error (-1,"Cannot set current directory to [%s], [%s]", ipath, RIM_FERR);
}


//
// returns 1 if there is another server running, 0 if not. If op is RIM_CHECKLOCK, then do not lock (client), just check.
// If it is RIM_LOCK, lock the file (only server does this).
// A file is locked to achieve one-app-one-server.
//
static rim_num srvhere(rim_num op) {
    char lpath[RIM_MAX_OS_UDIR_LEN];
    rim_dir (RIM_DIR_LOCK, lpath, sizeof(lpath), rimapp, NULL);
    if (op == RIM_CHECKLOCK) 
    {
...
static rim_num testlockfile (char *fname) 
{
    ...
    if (fcntl(fd, F_GETLK, &lock) == -1) { //error
        log_msg("Test lock file [%s] failed [%s]", fname, RIM_FERR);
        close(fd);
        return 0;
...
static void unlockfile (rim_num lf) {
    ...
    log_msg("UnLock file result [%d], [%s]", unl, RIM_FERR);
}
...
static rim_num lockfile (char *fname, rim_num *lf) {
    ...
    *lf = open(fname, O_RDWR | O_CREAT, 0600);
    ...
    if (fcntl(*lf, F_SETLK, &lock) == -1) {
        log_msg("Lock file [%s] failed [%s]", fname, RIM_FERR);
...
static void tokarg() {
    ...
    rim_num quote;
    rim_num carg = 0;
    RIM_ANN (xarg[carg++] = strdup (command)); // avoid const issue
    ...
    while (1) {
        ...
        if (carg >= RIM_MAX_ARGS - 1) exit_error (-1,"Too many arguments specified for service processes [%ld]", carg);
    }
...
// If dest is RIM_MLOG, then if showinfo is 1, messages still go through even if logging is off.
// islogging is turned on *only* in resident mrim process.
//
static void log_msg0(rim_num dest, char *format, ...) {
    time_t t;
    time(&t);
    if (islogging) printf("%ld: %s: ", (rim_num)getpid(), strtok (ctime(&t), "\n"));
    if (islogging == 0 && dest == RIM_MLOG && showinfo == 0) return;
    va_list args;
...
static void exit_error(int ec, char *format, ...) {
    time_t t;
    time(&t);
    fprintf(stderr, "%ld: %s: Error: ", (rim_num)getpid(), strtok (ctime(&t), "\n"));
    va_list args;
    va_start (args, format);
...
// (such as if it was terminated, or otherwise we can't get it), then st is RIM_BAD_EXIT (which is 1000, not a valid exit status, thus signalling it never exited)
// st is set only if process does not exist any more, so if it does exist, it's RIM_BAD_EXIT
//
static pid_t whatisprocess(pid_t p, int *st) {
    int pstat = 0; // waitpid may not set anything, so it may be random (bad)
    rim_num pid = waitpid (p, &pstat, WNOHANG);
    if (st != NULL) *st = RIM_BAD_EXIT;
...
//
// Stop processes. If sig is RIM_QUIT, all srvc processes and mrim stop (total quit). If RIM_STOP, mrim continues running,
// while srvc processes are stopped. If RIM_STOPONE, stop one srvc process only (for adaptive downsizing).
//
static void handlestop(rim_num sig) {
    if (sig == RIM_QUIT || sig == RIM_STOP || sig == RIM_STOPONE ) {
        rim_num i;
        if (sig == RIM_QUIT) log_msg("Terminating service processes and the process manager (me)");
        if (sig == RIM_STOP) log_msg("Terminating service processes only");
        if (sig == RIM_STOPONE) log_msg("Terminating single service processes only");
        rim_num pkilled = 0;
        for (i = 0; i < num_process; i++) {
            ...
            if (p == plist[i]) { // means status not changed, child is running
                ...
                log_msg ("Child either died or PID is not a child [%s]", RIM_FERR);
            ...
            if (sig == RIM_STOPONE && pkilled == 1) break;
        }
        //sleep(1);
    }
    if (sig == RIM_QUIT) { // kill self if term signal
        munmap(rimmem, sizeof(shbuf));
        // We do not unlink shared memory as there's really no need for it, and in edge cases of timing, it can cause issues
        //shm_unlink (RIM_SHMEM);
...
static void getshm() {
    ...
    char shmem[NAME_MAX];
    snprintf (shmem, sizeof(shmem), "%s_%s", RIM_SHMEM, rimapp);
    ggmemid = shm_open (shmem, O_CREAT|O_RDWR, 0600);
    if (ggmemid == -1) exit_error (-1,"Cannot create shared memory [%s]", RIM_FERR);
    if (ftruncate (ggmemid, sizeof (shbuf)) == -1) exit_error (-1,"Cannot set shared memory size [%s]", RIM_FERR);
    rimmem = mmap (NULL, sizeof(shbuf), PROT_READ|PROT_WRITE, MAP_SHARED, ggmemid, 0);
    if (rimmem == MAP_FAILED) exit_error (-1,"Cannot get shared memory [%s]", RIM_FERR);
    atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst);
}
...
//
static void checkshm(bool is_running_server) {
    rim_num command = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
    rim_num data1 = atomic_load_explicit(&(rimmem->data1), memory_order_seq_cst);
    if (command == RIM_COMMAND) {
        log_msg ("Received command [%ld]", data1);
        // we don't send a response, just make sure we don't do it again
        if (data1 == RIM_RESTART) {
            ...
            atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst);
        } else if (data1 == RIM_START) {
            temp_no_restart = 0;
            log_msg ("Before process up");
            processup(0);
            log_msg ("After process up");
            atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst);
        } else if (data1 == RIM_STATUS) {
            atomic_store_explicit (&(rimmem->data1) , RIM_OKAY, memory_order_seq_cst);
            atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst);
        } else if (data1 == RIM_OKAY_STARTED) {
            ...
            atomic_store_explicit (&(rimmem->command) , RIM_DONESTARTUP, memory_order_seq_cst);
        } else if (data1 == RIM_OKAY_RUNNING) {
            if (is_running_server) return; // this is client trying to start the server and it's child should answer this; not the running server
            atomic_store_explicit (&(rimmem->command) , RIM_DONESTARTUP0, memory_order_seq_cst);
        } else if (data1 == RIM_STOP) {
            handlestop (RIM_STOP);            
            temp_no_restart = 1;
            atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst);
        } else if (data1 == RIM_QUIT) {
            ...
            {
                atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst); // this must come BEFORE handling quit as that will delete memory!
                handlestop (RIM_QUIT); // exits, no return
            }
        }
        else
        {
            ...
            atomic_store_explicit (&(rimmem->command) , RIM_DONEBAD, memory_order_seq_cst);
        }
    }
}
...
static void wait_server_end() 
{
    rim_num tries = RIM_TRIES;
    while (tries-- >= 0) 
    {
        if (srvhere(RIM_CHECKLOCK) != 1) {
            break;
        }
        sleepabit (mslp);
    }
...
static void cli_getshm(char *comm, rim_num byserver) {
    ...
    if (byserver == 0)
    {
        ...
        if (ggmemid == -1) 
        {
            ...
            if (strcmp (comm, "quit")) exit_error (-1,"There is no active mrim server named [%s]", rimapp);
    ...
    // if we checked this for server, then lock would be removed with srvhere(RIM_CHECKLOCK)
    if (byserver == 0 && srvhere(RIM_CHECKLOCK) == 0) {
    ...
    if (byserver == 0) {
        ...
        // but mrim client is an administrative one; there is not supposed to be more than one person doing this. It is
        ...
        while (tries-- >= 0) {
            rim_num command = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
            if (command == RIM_COMMAND) {
                sleepabit (mslp); // normally not RIM_COMMAND, so null virtually always
            } else break;
        }
        rim_num command = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
        if (command == RIM_COMMAND) exit_error (-1,"Cannot contact server: %s", servererr);
    }
    // Now issue the actual command, know the server is in a good state (most likely)
if (!strcmp (comm, "start")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_START, memory_order_seq_cst);
else if (!strcmp (comm, "stop")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_STOP, memory_order_seq_cst);
else if (!strcmp (comm, "restart")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_RESTART, memory_order_seq_cst);
else if (!strcmp (comm, "quit")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_QUIT, memory_order_seq_cst);
else if (!strcmp (comm, "status")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_STATUS, memory_order_seq_cst);
else if (!strcmp (comm, "okay_started")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_OKAY_STARTED, memory_order_seq_cst);
else if (!strcmp (comm, "okay_running")) 
            atomic_store_explicit (&(rimmem->data1) , RIM_OKAY_RUNNING, memory_order_seq_cst);
else exit_error (-1, "Unknown client command [%s]", comm);

    atomic_store_explicit (&(rimmem->command) , RIM_COMMAND, memory_order_seq_cst);
    log_msg ("Sent command [%s], by server [%ld]", comm, byserver);
    tries = RIM_TRIES;
    rim_num command;
    rim_num data1;
    while (tries-- >= 0) {
        command = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
        data1 = atomic_load_explicit(&(rimmem->data1), memory_order_seq_cst);
        if (command != RIM_COMMAND) {
            if (command == RIM_DONEBAD) exit_error (-1,"Unknown command [%s]", comm);
            // if asked for status, print what server responded. Use stdout, as no dup2 took place.
            if (!strcmp (comm, "status")) out_msg ("%s\n", data1==RIM_OKAY?"okay":"not running");
    ...
    rim_num ccom = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
    atomic_store_explicit (&(rimmem->command) , RIM_DONE, memory_order_seq_cst); // always reset so that previous command doesn't stay in memory if previous server went away
                            // for example if previous server didn't exist or timed out, RIM_COMMAND may stay and confuse the
                            // following client
...
static void start_child (char *command, rim_num pcount) {
    ...
    if (stat(command, &sbuff) != 0) { msg_notfound(RIM_FERR); return;}
    ...
    while (1) { // access may be temporarily denied, wait 5 seconds
        ...
        if (access(command, X_OK) != 0) {
            if (retries <= 0) { log_msg ("File not accessible to start [%s]", RIM_FERR); return;}
            sleepabit (100);
            continue;
        } else break;
    }
    //
    if (commtime == NULL) RIM_ANN (commtime = (struct timespec*)malloc (sizeof(struct timespec)));
    ...
    //
    // Fork for a final child, to prevent daemon processes (RimStone SERVICE programs) from ever being able to acquire controlling terminal
    fres = fork();
    if (fres == 0) {
        ...
        if (res == -1 || ppid != getppid()) {
            exit_error (-1,"Cannot set parent death signal [%s]", RIM_FERR);
        }


        // umask to 0. For rimstone, it doesn't really matter because it sets own umask. In general, a good idea
        umask (0);
        ...
        close(2);
        if (dup (sockfd) != 0) {
            exit_error (-1,"Cannot dup socket to 0 [%s]", RIM_FERR);
        }
        if (dup (sockfd) != 1) {
            exit_error (-1,"Cannot dup socket to 1 [%s]", RIM_FERR);
        }
        if (dup (sockfd) != 2) {
            exit_error (-1,"Cannot dup socket to 2 [%s]", RIM_FERR);
        }
        ...
        // Note that we put environment in rim_env - we intentionally clean up environment so service programs do not get confused with
        // possibly random environment variables. TODO: add environment variables to this list through startup mrim process (somehow, not
        ...
        rim_num tot_env = (1)+1; // for 1 env var plus one NULL at the end
        rim_env = (char**)malloc(sizeof(char*) * tot_env);
        char o_sil[100];
        snprintf (o_sil, sizeof(o_sil), "RIM_SILENT_HEADER=%s", silent?"yes":"no");
        rim_num curr_env = 0;
        rim_env[curr_env++] = o_sil;
        rim_env[curr_env] = NULL;
        if (curr_env >= tot_env) exit_error (-1,em);
        //
        // Execute service process
        //
        if (execvpe(command, xarg, rim_env)) exit(-1);
    } else if (fres == -1) {
        // ERROR
        exit_error(-1,"Cannot fork child process [%s]", RIM_FERR);
    } else {
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "ft:l:dgnu:eir:xs:a:m:vc:p:w:hz", opts, &oind)) != -1) {
        switch (c) {
            ...
            case 0:
                ...
                break;
            case 'v':
                out_msg ("RimStone Service Manager version [%s].", RIM_PKGVERSION);
                exit (0);
                break;
            case 'f':
                fg = 1;
                break;
            case 'u':
                RIM_ANN (run_user = strdup (optarg));
                break;
            case 'n':
                dnr = 1;
                break;
            ...
            case 'x':
                ...
                break;
            case 'a':
                RIM_ANN (parg = strdup (optarg));
                break;
            case 'm':
                RIM_ANN (client_msg = strdup (optarg));
                break;
            case 'h':
                usage(0);
                break;
            case 'r':
                RIM_ANN (proxy_grp = strdup (optarg));
                struct group *grp;
                if ((grp = getgrnam(proxy_grp)) == NULL) 
                {
                    exit_error (-1,"Unknown group [%s], [%s]", proxy_grp, RIM_FERR);
                }
    ...
    RIM_ANN (plist = (pid_t*)calloc(maxproc, sizeof(pid_t)));
    rim_num i;
    for (i = 0; i < maxproc; i++) plist[i] = -1; // set as inactive processes
    
    // get app name
    if (optind <= argc - 1) RIM_ANN (rimapp = strdup (argv[optind++]));
    else {
        if (rimapp[0] == 0) exit_error (-1,"Application name must be specified as an argument [%ld,%ld]", optind, argc-1);
    }
    rim_num l;
    if ((l = strlen (rimapp)) > 30) exit_error (-1,"Application name [%s] too long", rimapp);
    for (i = 0; i < l; i++) 
    {
        if (isalnum(rimapp[i])) continue;
        if (rimapp[i]=='_') continue;
        if (rimapp[i] == '-') { continue; }
        exit_error (-1,"Application name can be comprised only of underscore, hyphen, digits and characters, found [%s]", rimapp);
    }
    if (!isalpha(rimapp[0])) exit_error (-1,"Application name must start with alphabetic character, found [%s]", rimapp);
    ...
    if (run_user[0] == 0) {
        ...
        if ((pwd = getpwuid (geteuid())) == NULL) exit_error (-1,"Cannot find current user [%s]", RIM_FERR);
    } else {
        if (initit == 0) exit_error (-1,"User (-u) can be specified only when initializing (-i)");
        if ((pwd = getpwnam(run_user)) == NULL) exit_error (-1,"Cannot find user [%s], [%s]", run_user, RIM_FERR);
        free (run_user); // it was allocated by strdup in options processing, will be allocated again, just below
    }
    RIM_ANN (run_user = strdup (pwd->pw_name));
    run_user_id = pwd->pw_uid;
    run_user_grp_id = pwd->pw_gid; // group of current user

    // Check if -i options, and if it is, initialize app. Every RimStone app must have this done.
    // Recommended for all apps.
    if (initit == 1) {
        //
        // BEGIN ROOT - this section is the only time mrim is allowed as root
        // This is mrim -i setup, the only time we may need root privs. Most of the time we don't
        ...
        //
        // NOTE all rim_dir has run_user as the last argument, if not, all dirs would be /root when we run sudo mrim
        // and we do that when we need to set socket to group ID of some other user (like apache GUID etc)
        //
        //
        // create .rimstone/apps dir, 711 so socket can pass through
        //
        rim_dir (RIM_DIR_APPS, ipath, sizeof(ipath), NULL, run_user);
        initdir (ipath, 0711, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>
        //
        rim_dir (RIM_DIR_APPNAME, ipath, sizeof(ipath), rimapp, run_user);
        owned (ipath, run_user_id); // make sure no one else already took this
        initdir (ipath, 0711, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/app dir
        //
        rim_dir (RIM_DIR_APP, ipath, sizeof(ipath), rimapp, run_user);
        owned (ipath, run_user_id); // make sure no one else already took this
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // Make creation artifact, used to recreate the application
        // Must be aftere the above RIM_DIR_APP is created or the dir may not exist for the artifact here
        //
        char artname[RIM_MAX_OS_UDIR_LEN];
        rim_dir (RIM_DIR_ART, artname, sizeof(artname), rimapp, run_user);
        ...
        fclose (art);
        //
        // create ./rimstone/apps/<app>/.bld dir
        //
        rim_dir (RIM_DIR_BLD, ipath, sizeof(ipath), rimapp, run_user);
        owned (ipath, run_user_id); // make sure no one else already took this
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/file
        //
        rim_dir (RIM_DIR_FILE, ipath, sizeof(ipath), rimapp, run_user);
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/file/t
        //
        rim_dir (RIM_DIR_TMP, ipath, sizeof(ipath), rimapp, run_user);
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/trace
        //
        rim_dir (RIM_DIR_TRACE, ipath, sizeof(ipath), rimapp, run_user);
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/db
        //
        rim_dir (RIM_DIR_DB, ipath, sizeof(ipath), rimapp, run_user);
        initdir (ipath, 0700, run_user_id, run_user_grp_id);
        //
        // create .rimstone/apps/<app>/.sock (with group ownership of reverse proxy)
        //
        ...
        if (proxy_grp[0] != 0) {
            struct group *grp;
            if ((grp = getgrnam(proxy_grp)) == NULL) exit_error (-1,"Unknown group [%s], [%s]", proxy_grp, RIM_FERR);
            proxy_grp_id = grp->gr_gid;
            //
            rim_dir (RIM_DIR_SOCK, ipath, sizeof(ipath), rimapp, run_user);
        ...
        rim_dir (RIM_DIR_MEM, ipath, sizeof(ipath), rimapp, run_user);
        // do not recreate file, as that leaves current server's memory in a limbo - will never receive anything
        if (stat(ipath, &sbuff) != 0) {
            rim_num f;
            if ((f = open (ipath, O_CREAT | O_RDWR | O_TRUNC, 0700)) == -1) exit_error (-1,"Cannot create mem file [%s] [%s]", ipath, RIM_FERR); else close(f);
        }
        if (chown (ipath, run_user_id, run_user_grp_id)!= 0) exit_error (-1,"Cannot change the ownership/group of shmem key file [%s], [%s]", ipath, RIM_FERR);
        if (chmod(ipath, 0700) != 0) exit_error (-1,"Cannot set permissions for shmem key file [%s]", RIM_FERR);
        exit (0);
        //
    ...
    // check sanity
    if ((unix_sock == 0 && port == 0) || rimapp[0] == 0) {
        usage(-1);
    }

    rim_dir (RIM_DIR_BLD, ipath, sizeof(ipath), rimapp, NULL);
    // use ./command for something in current directory or a full path
    if (command[0] == 0) {
        // if no command, assume standard RimStone format - in rimstone's bld directory
        RIM_ANN (command = (char*)malloc (RIM_MAX_FILELEN+10)); // +10 for gcc to not complain about below snprintf
        snprintf (command, RIM_MAX_FILELEN+10,  "%s/%s.srvc", ipath, rimapp);
    } else {
        char *s = strchr (command, '/');
        if (s == NULL) {
            // if command without path, place the file in rimstone's bld directory
            char *nc;
            RIM_ANN (nc = (char*)malloc (RIM_MAX_FILELEN + 10)); // +10 for gcc about below snprintf
            snprintf (nc, RIM_MAX_FILELEN+10, "%s/%s", ipath,  command);
...
// check if command exists and is executable
    ...
    char logn[RIM_MAX_OS_UDIR_LEN];
    rim_dir (RIM_DIR_MGRG, logn, sizeof(logn), rimapp, NULL);
    ...
    atomic_store_explicit (&(rimmem->command) , RIM_OKAY, memory_order_seq_cst); // clear up command flag, so any leftover is not confusing the processes below
    if (deadres == 0) {
// so we can attach to process with gdb without sudo
        ...
        // not as important for RimStone, as it handles all signals, in general a good idea
        if (fg == 0) { // create session only if running in background
            if ((sid = setsid()) < 0) {
                exit_error (-1,"Cannot create new session group [%s]", RIM_FERR);
            }
        }
        ...
        if (fg == 0) { // redirect stdin,out,err only if background mode, otherwise print to console
            ...
            if (logfile == NULL) exit_error (-1,"Cannot open log file [%s]", RIM_FERR);
            // close stdin,stdout,stderror, and redirect them to /var/log/rim
            close(0);
            close(1);
            close(2);
            if (dup (fileno(logfile)) != 0) { // logfileile to stdin? we will never read; must have it so children can use it to dup socket!
                exit_error (-1,"Cannot dup logfile to 0 [%s]", RIM_FERR);
            } 
            if (dup (fileno(logfile)) != 1) {
                exit_error (-1,"Cannot dup logfile to 1 [%s]", RIM_FERR);
            }
            if (dup (fileno(logfile)) != 2) {
                exit_error (-1,"Cannot dup logfile to 2 [%s]", RIM_FERR);
            }
        ...
        if (srvhere(RIM_LOCK) == 1) {
            ...
            exit_error (2,"The RimStone process manager for application [%s] is already running", rimapp);
        }

        log_msg ("RimStone Service Manager v%s starting", RIM_PKGVERSION);
        log_msg ("Command [%s], port [%ld], workers [%ld], user [%s], name [%s], sleep [%ld], minmaxset [%ld], min-workers [%ld], max-workers [%ld]", command, port, num_to_start_min, run_user, rimapp, mslp, minmaxset, num_to_start_min, maxproc);
        ...
        if (unix_sock == 1) {
            ...
            char ipath[RIM_MAX_OS_UDIR_LEN];
            rim_dir (RIM_DIR_SOCKFILE, ipath, sizeof(ipath), rimapp, NULL);
            strncpy (servaddr.sun_path, ipath, sizeof(servaddr.sun_path)); 
            servaddr.sun_path[sizeof(servaddr.sun_path)-1] = 0;
            //
            if (unlink(servaddr.sun_path) != 0) if (errno != ENOENT) exit_error (-1,"Cannot unlink unix domain socket file [%s]", RIM_FERR);
            if (bind(sockfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) != 0) exit_error (-1,"Cannot bind unix domain socket [%s], [%s]", servaddr.sun_path, RIM_FERR);
            if (proxy_grp[0] != 0) {
                // create .rimstone/apps/<app>/.sock
            ...
            if (bind(sockfd, (struct sockaddr *) &servaddr, sizeof(servaddr)) != 0) exit_error (-1,"Cannot bind socket [%s]", RIM_FERR);
        }
        log_msg ("After binding socket [%ld]", opid);
        
        if ((rv = listen(sockfd, maxblog)) != 0) {
            exit_error (-1,"Cannot listen on socket [%s]", RIM_FERR);
        }
        ...
        num_process = maxproc;
        signal(SIGCHLD, SIG_IGN); // avoid defunct processes when children killed - we do not expect return value
                                  // this is for actual RimStone server processes
        while (--init_start >= 0) {
            start_child (command, pcount++);
        }
        if (fg == 0) cli_getshm ("okay_started", 1); // nobody to communicate with unless forked first
        log_msg ("RimStone Service Manager v%s successfully started", RIM_PKGVERSION);
        ...
        exit_error (-1,"Could not start RimStone Service Manager for [%s], [%s]", rimapp, RIM_FERR);
    } else {
        #define RIM_SRV_RUNNING "RimStone Service Manager [%s] for [%s] is already running. If you want to restart it, stop it first (-m quit), then start it"
#define RIM_SRV_STARTED "RimStone Service Manager [%s] for [%s] successfully started"
#define RIM_SRV_NOSTART "Could not start RimStone Service Manager for [%s],[%ld],[1], log file [%s]"
        ...
        while (tries-- >= 0) {
            ...
            if (!(p == deadres || p == -deadres))  // if resident process started above is no longer alive
            {
                if (st == 2) 
                {
                    ...
                    // it will be either this or the RIM_SRV_RUNNING below, so this is to make sure we get that message
                    atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
                    checkshm(false);  // reset server memory
                    out_msg (RIM_SRV_RUNNING, RIM_PKGVERSION, rimapp); 
                    exit(1);
                } 
                else 
                {
                    ...
                    atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
                    checkshm(false);  // reset server memory
                    exit_error (-1,RIM_SRV_NOSTART, rimapp, tries, logn);
                }
            }
            ...
            rim_num command = atomic_load_explicit(&(rimmem->command), memory_order_seq_cst);
            checkshm(false); // this must be AFTER getting rimmem->command, as that's what's checked 
            // This says ONLY that a manager daemon has started. The actual processes start asynchronously after this!!!
            if (command == RIM_DONESTARTUP) {out_msg (RIM_SRV_STARTED, RIM_PKGVERSION, rimapp); exit(0);}
            else if (command == RIM_DONESTARTUP0) {out_msg (RIM_SRV_RUNNING, RIM_PKGVERSION, rimapp); exit(1);}
            sleepabit (mslp);
        }
    ...
    // monitor processes, exit is possible with quit command from mrim client (-m option)
    while (1) {
        ...
        if (!(temp_no_restart == 1 || dnr == 1)) {
            ...
            if (modreload == 1 && checkmod() == 1) { sleepabit(500); handlestop (RIM_STOP); }
            processup(0);
        }
