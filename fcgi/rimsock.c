...
//
int rim_fcgi_read(int f, char * buf, size_t len)
{
...
//
int rim_fcgi_write(int f, char * buf, size_t len)
{
...
//
int rim_fcgi_accept(int listen_sock, int sec_timeout)
{
    int socket = -1;
    union {
    ...
    // make sure the read (we do read then write for a request in RimStone, and then close!)
    ...
    if (sa.in.sin_family == AF_INET || sa.in.sin_family == AF_UNIX) 
    {
        ...
        {
            // RimStone generally deals with request-replies typically over Unix sockets
