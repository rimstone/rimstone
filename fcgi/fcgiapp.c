...
static int FCGX_GetChar(FCGX_Stream *stream);
...
int FCGX_FPrintF(FCGX_Stream *stream, const char *format, ...)
{
    ...
    va_start(ap, format);
#ifdef DEBUG
    char buff[64]; // use reasonably sized stack buffer to print out most messages, or allocate if not enough later
#else
    ...
    if (tot_written >= (gg_num)sizeof(buff))
    {
        ...
        result = FCGX_PutStr(dbuff, tot_written, stream);
    }
    else
    {
        va_end(ap);
        result = FCGX_PutStr(buff, tot_written, stream);
    }
    return result;
}
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    for (;;) {
        ...
        }
        {
            ...
            PutParam(reqDataPtr->paramsPtr, strdup(roleStr));
        }
