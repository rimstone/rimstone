...
// -rdynamic (most likely) must be used. RimStone always does that automatically.
//
// This uses libbacktrace (see LICENSE in btrace directory in RimStone source, as well License 
...
//
#define RIM_MAX_MSG_LEN 2048 // max length of message with backtrace. It goes from deepest levels first, so it will catch the most important bits
#define RIM_MAX_STACK_FRAMES 150

static char *expla = ""; // error explanation
// outputting data
static char rim_emsg[RIM_MAX_MSG_LEN];  // message that's output
static rim_num rim_emsg_pos = 0; // current position where we're writing into the message
//
...
volatile rim_num rim_end_program=0; // when SIGTERM is received, set this to 1 to exit gracefully
extern rim_num rim_in_request; // 1 if we're processing request when signal happened, 0 if not
...
static void rim_add_msg (char *add, rim_num *pos, char *to, rim_num avail);
static void rim_itos (int n, char *s, rim_num avail);
char *curr_err; // current error message from the caller
static rim_num first_output = 1; // special output for the very first output of an error (meaning first in a list of lines 
...
{
    // init stack just before getting it. We don't do it in signal handler, because we can call rim_get_stack() outside of it, say for
    // RimStone own exceptions
    ...
    //
    //
    rim_gen_write (true, rim_emsg, rim_emsg_pos); // write to stderr - first write the stack

    // Construct the file name to output to
    char btrace_cur[RIM_MAX_FILE_NAME+50]; // file name of the current stack trace (pid-1,2,3...) so one for each incident, easier to use
    rim_num btrace_pos = 0; // position in btrace_cur string
    static rim_num btrace_counter  = 0; // counter of stack trace incidents, 1,2,3...
    // get the counter as string
    char counter[30];
    rim_itos (btrace_counter, counter, sizeof(counter));
    // file name, generic trace $HOME/.rimstone/apps/<app>/app/trace/bt.PID-<incident>
    rim_add_msg (btrace_cur, &btrace_pos, btrace_path, sizeof(btrace_cur));
    rim_add_msg (btrace_cur, &btrace_pos, "-", sizeof(btrace_cur));
    rim_add_msg (btrace_cur, &btrace_pos, counter, sizeof(btrace_cur));
    ...
    int written = write (bwrite, rim_emsg, rim_emsg_pos); 
    RIM_UNUSED(written); // if it doesn't work, we do nothing as this isn't the functionality; it's a best-effort try to record
    close (bwrite);
...
static void rim_itos (int n, char *s, rim_num avail)
{
    ...
    if (neg) n = -n;
#define RIM_MAX_DIGITS 25
    if (avail <= RIM_MAX_DIGITS) {s[0] = 0; return;}
    int last = RIM_MAX_DIGITS-2;
    do
    {
        int mod = n % 10;
        n = n / 10;
    ...
    memmove (s+sign_adj, s+last, RIM_MAX_DIGITS-2-last+1);
    s[sign_adj+RIM_MAX_DIGITS-2-last+1] = 0;
}
...
static void bt_handle_error (void *data, const char *msg, int error_number) 
{
    ...
    else 
    {
        ...
        rim_add_msg (rim_emsg, &rim_emsg_pos, "Backtrace error ", sizeof(rim_emsg));
        char lnum[50];
        rim_itos (error_number, lnum, sizeof(lnum));
        rim_add_msg (rim_emsg, &rim_emsg_pos, (char*)lnum, sizeof(rim_emsg));
        rim_add_msg (rim_emsg, &rim_emsg_pos, ": ", sizeof(rim_emsg));
        rim_add_msg (rim_emsg, &rim_emsg_pos, (char*)msg, sizeof(rim_emsg));
        rim_add_msg (rim_emsg, &rim_emsg_pos, "\n", sizeof(rim_emsg));
        // this will be displayed in rim_get_stack() since this is called  from backtrace_full(), also written to a file there
  }
}
...
static int bt_handle_info (void *data, uintptr_t pc, const char *srcfile, int lnum, const char *function) 
{
    RIM_UNUSED(data);
    RIM_UNUSED(pc);
    static rim_num sframes = 0; // how many stack frames we process?
    ...
    if (srcfile != NULL && function != NULL && lnum != 0)  // sanity check
    {
        ...
        // Do not output anything for functions here, in rimrt.c and for main/libc. This is just to remove info that isn't very useful.
        ...
        if (!(
                ...
                || 
                (!strcmp (filename, "rimrt.c") && !strcmp (function, "rim_write_ereport")) 
                || 
                (!strcmp (filename, "rimrt.c") && !strcmp (function, "rim_dispatch_request")) 
                || 
          ...
          ))
        {
            ...
            { 
                ...
                first_output = false;
                rim_add_msg (rim_emsg, &rim_emsg_pos, "\n--- ", sizeof(rim_emsg));
                rim_add_msg (rim_emsg, &rim_emsg_pos, curr_err, sizeof(rim_emsg));
                rim_add_msg (rim_emsg, &rim_emsg_pos, ". Stack backtrace of the error:\n", sizeof(rim_emsg));
            }
            // output backtrace line
            // this will be displayed in rim_get_stack() since this is called  from backtrace_full(), also written to a file there
            //
            bool is_rim = (strstr (filename,".rim") != NULL);
            // If this is rim source, put * in front, this makes it visually easier to see
            // otherwise just plain indent
            if (is_rim) rim_add_msg (rim_emsg, &rim_emsg_pos, "     *", sizeof(rim_emsg));
            else rim_add_msg (rim_emsg, &rim_emsg_pos, "      ", sizeof(rim_emsg));
            /// convert line num to string
            char lnum_str[50];
            rim_itos (lnum, lnum_str, sizeof(lnum_str));
            // display filename:line in func()
            rim_add_msg (rim_emsg, &rim_emsg_pos, (char*)filename, sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos, ":", sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos, (char*)lnum_str, sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos,  " in ", sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos, (char*)function, sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos,  "()", sizeof(rim_emsg));
            rim_add_msg (rim_emsg, &rim_emsg_pos, "\n", sizeof(rim_emsg));
            if (sframes++ > RIM_MAX_STACK_FRAMES) return  1; // this means stop doing it, too many; meaning backtrace_full() will stop after this
        }
    }
...
static void signal_handler(int sig)
{
    ...
    // make sure no surprises with longjumps, disable them right away. rim_done_err_setjmp is used
    // for report-error to go to the next request, and rim_done_setjmp for exit-service. Neither should
    // potentially do these, because this is fatal and must exit, so we allow no jumps elsewhere that would
    // prevent fatal exit.
    rim_done_err_setjmp = 0;
    rim_done_setjmp = 0;


    // set to make sure rim_report_error does not exit, but lets this function go through the end to report
    ...
    switch(sig)
    {
        ...
        case SIGTERM:
            ...
            if (rim_in_request == 0) 
            {
                expla="Caught SIGTERM: request for graceful shutdown, shutting down now as RimStone is not processing a request";
                break; // go down, print out stack and end it
            }
            else
            {
                expla= "Caught SIGTERM: request for graceful shutdown, RimStone will shutdown once a request is processed";
            }
...
#ifdef RIM_COMMAND
    rim_finished_output = 0;
#endif


    rim_get_stack(expla);
     // NOTE: this is in signal handler, only serious RimStone bugs basically come here. We just dump stack and exit. Doing more than
     ...
     //
     // A report-error (whether it's a statement, or if RimStone called it because of memory safety guard or like that), will NEVER be calling here
     // That goes to  v1.c (in do_printf) or _rim_report_error (if memory safety issue or status etc). Those will continue processing (if service).
     //

    // exit and quit, cannot continue
    RIM_FATAL ("Exiting because of the signal: [%s]", expla);
}
...
static void set_signal_handler()
{
    ...
    if (sigaction(SIGABRT, &psa, NULL) == -1) RIM_FATAL ("Cannot set ABRT signal handler");
    if (sigaction(SIGFPE,  &psa, NULL) == -1) RIM_FATAL ("Cannot set FPE signal handler");
    if (sigaction(SIGILL,  &psa, NULL) == -1) RIM_FATAL ("Cannot set ILL signal handler");
    if (sigaction(SIGSEGV, &psa, NULL) == -1) RIM_FATAL ("Cannot set SEGV signal handler");
    if (sigaction(SIGFPE, &psa, NULL) == -1) RIM_FATAL ("Cannot set SEGFPE signal handler");
    if (sigaction(SIGBUS, &psa, NULL) == -1) RIM_FATAL ("Cannot set BUS signal handler");
    if (sigaction(SIGTERM, &psa, NULL) == -1) RIM_FATAL ("Cannot set TERM signal handler");
    if (sigaction(SIGHUP, &psa, NULL) == -1) RIM_FATAL ("Cannot set HUP signal handler");
    //
    // ignore these
    //
    ...
    if (sigaction(SIGINT, &ipsa, NULL) == -1) RIM_FATAL ("Cannot ignore SIGINT"); // ctrl c
    if (sigaction(SIGPIPE, &ipsa, NULL) == -1) RIM_FATAL ("Cannot ignore SIGPIPE"); // broken pipe
    if (sigaction(SIGUSR1, &ipsa, NULL) == -1) RIM_FATAL ("Cannot ignore SIGUSR1"); // no meaning in rim yet
    if (sigaction(SIGUSR2, &ipsa, NULL) == -1) RIM_FATAL ("Cannot ignore SIGUSR2"); // no meaning in rim yet
...
//
// This is what's called by generated RIM program at the beginning
// to enable catchng signals and dumping human-readable stack in backtrace file
// btrace is a partial file name (with path), i.e. /$HOME/.rimstone/apps/<app>/app/trace/bt.<PID> and we add -1, -2.. to it
