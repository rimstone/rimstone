...
void rim_write_msg(rim_msg *msg, char *key, char *value)
{
    ...
    else
    {
        ...
        if (msg->tot < msg->len+keyl+valuel+RIM_MSG_ADD_LEN) msg->data = rim_realloc (rim_mem_get_id(msg->data), msg->tot = msg->len+keyl+valuel+RIM_MSG_ADD_LEN + msg->addinc); 
    }
        ...
        else
        {
            ...
            if (value != NULL) 
            {
                ...
                *value = rim_mem_add_const (ivalue-RIM_ALIGN, value_len+1); 
                //*value = rim_strdupl (ivalue,0, value_len);            
            }
            return RIM_OKAY;
        }
    }
